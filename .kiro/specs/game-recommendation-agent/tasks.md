...
- [x] 14. Store deals — agent-driven, principle-based (no per-store tool)
  - [x] 14.1 Region auto-detection + today, surfaced to the agent
    - The user's **region** (and so currency) is detected from the **browser timezone** (`Intl…timeZone` via `streamlit-js-eval`, mapped `Europe/Copenhagen → Denmark` in `ui.bootstrap`) — more reliable than `Accept-Language` (a Dane may run en-GB) or a server's IP/AWS region (the datacenter). Resolution: explicit `DEALS_REGION` › timezone › `Denmark`. Both the region and **today's date** are injected into the system prompt (`agent.runtime.system_prompt_for_region(region, today)` from `bootstrap.build_app`), so the agent reads the right regional store/currency without asking and can reject expired sales. `Config.deals_region` is optional; `get_context` rebuilds when the timezone resolves. Tests: region precedence + date in `test_bootstrap`, timezone map in `test_ui_smoke`.
    - _Requirements: 7.x, 10.3_
  - [x] 14.2 Deals as a general capability + principle-based instructions
    - **Decision (user, after extensive testing): keep the agent free; no per-store deal code.** A dedicated `find_deals` tool / `agent/deals.py` (store URL templates, locale maps, an Xbox price regex, even per-store JSON-API clients) was prototyped and **removed** — it baked an empirical scraping process into a tool and only worked for Xbox. Instead, `web_search` gained `deep` (Tavily advanced + bounded `raw_content`, to read a store page's actual prices) and `site` (domain restrict); the *learnings* live in the system prompt as **principles**, not recipes: a price must be the CURRENT price on the user's **own regional store in their currency** (never a converted "global"/US view); prefer the exact country, else fall back to a **nearby European country, clearly labelled as approximate** (a UK eShop price for a Dane is a fair proxy; a US one is not); prefer the official store, else a reputable region-locked tracker — never grey-market key resellers (AllKeyShop/Eneba/gg.deals); ignore subscriber prices (Game Pass/EA Play); name the source + region; not-listed = not on sale; check end dates vs today; never guess a number. Empirical findings that shaped this (Xbox store page reads via deep search; Nintendo/Steam/PS pages don't expose prices to scraping; official APIs exist but were deliberately not wired) are recorded but **not** hard-coded — the agent finds and reasons over sources itself, the same way for every platform. Verified live on GameGusto for Xbox/PlayStation/Switch/Steam (real DKK where readable, labelled EU fallback or honest "check the store" otherwise; no fabricated/US-converted prices).
    - _Requirements: 7.x, 10.3, 11.4_
