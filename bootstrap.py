...
DynamoDB-backed memory, record sources, library assembly, recommender, and the
conversation orchestrator) from a :class:`~config.Config`. The future Streamlit
UI and the headless CLI both build on this, so the wiring lives in one place.
...
def build_app(config: Config, user_id: str = "default") -> AppContext:
    ...
    recommender = Recommender(bedrock, memory)
    orchestrator = AgentOrchestrator(
        mood_interpreter=MoodInterpreter(bedrock),
