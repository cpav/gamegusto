...
- **Tool**: A typed function the model may call during a turn (e.g. get/add/remove platforms, get/update library, import Gmail, enrich game, web search (incl. deep page reads for store deals), recent recommendations, save recommendation). Each Tool wraps an existing service and is declared to the model with a JSON-schema input. The Tool set is extensible without changing the loop.
